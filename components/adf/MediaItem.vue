...
<template>
    ...
    <div v-else-if="attachment.type === 'doc'" @click="modalOpen = true" class="relative inline-block w-80 h-60 p-4 dark:bg-slate-800 bg-slate-100 rounded-lg">
        <pre class="overflow-hidden h-50">{{doc}}</pre>
        <div class="absolute left-0 bottom-0 w-full dark:bg-slate-600 bg-slate-300 rounded-b-lg p-4">{{attachment.name}}</div>
    </div>
